...
final class IngestionService {
    ...
    func ingestCPSC(enrollmentURL: URL, contractsURL: URL, year: Int, month: Int) async throws {
        ...
        Logger.log("DEBUG: [CPSC] Starting ingestion for periodID: \(periodID) (\(year)-\(month))")
        ...
        Logger.log("DEBUG: [CPSC] [1/4] Ultra-Fast Enrollment Ingestion...")
        let eStart = Date()
        try fastBareMetalStream(url: enrollmentURL, table: "staging_enrollment", mapping: eMapping, filterEnrollment: true, skipRows: 1)
        Logger.log("DEBUG: [CPSC] Enrollment took \(Date().timeIntervalSince(eStart))s")
        
        Logger.log("DEBUG: [CPSC] [2/4] Contract Ingestion...")
        let cStart = Date()
        try fastBareMetalStream(url: contractsURL, table: "staging_contracts", mapping: cMapping, filterEnrollment: false, skipRows: 1)
        Logger.log("DEBUG: [CPSC] Contract took \(Date().timeIntervalSince(cStart))s")
        
        Logger.log("DEBUG: [CPSC] [3/4] Indexing...")
        let idxStart = Date()
        ...
        """)
        Logger.log("DEBUG: [CPSC] Indexing took \(Date().timeIntervalSince(idxStart))s")
        
        Logger.log("DEBUG: [CPSC] [4/4] Merging...")
        let mergeStart = Date()
        ...
        Logger.log("DEBUG: [CPSC] Merge took \(Date().timeIntervalSince(mergeStart))s")
        ...
        Logger.log("DEBUG: [CPSC] Total Records Merged: \(count.first?["c"] ?? 0)")
        Logger.log("DEBUG: [CPSC] --- OVERALL TIME: \(Date().timeIntervalSince(overallStart))s ---")
    }

    func ingestLandscape(url: URL, year: Int) async throws {
        Logger.log("DEBUG: [Landscape] Starting ingestion from \(url.lastPathComponent) for year \(year)")
        try store.database.execute(sql: "PRAGMA synchronous = OFF; PRAGMA journal_mode = OFF; PRAGMA cache_size = -1000000; PRAGMA temp_store = MEMORY; PRAGMA locking_mode = EXCLUSIVE;")
        ...
        if mapping.isEmpty { 
            Logger.log("DEBUG: [Landscape] WARNING: Could not detect column mapping for \(url.lastPathComponent)")
            return 
        }
        
        Logger.log("DEBUG: [Landscape] Column Mapping: \(mapping)")
        
        try fastBareMetalStream(url: url, table: "staging_landscape", mapping: mapping, filterEnrollment: false, skipRows: headerRowIndex + 1)
        
        Logger.log("DEBUG: [Landscape] Indexing staging table...")
        let idxStart = Date()
        try store.database.execute(sql: """
        ...
        Logger.log("DEBUG: [Landscape] Merging into final tables...")
        let mergeStart = Date()
        let sql = DBSchema.mergeLandscapeToFinal.replacingOccurrences(of: ":year", with: "\(year)")
        try store.database.execute(sql: "BEGIN TRANSACTION; \(sql) COMMIT;")
        Logger.log("DEBUG: [Landscape] Merge took \(Date().timeIntervalSince(mergeStart))s")
        
        let countRow = try store.database.query(sql: "SELECT COUNT(*) as c FROM landscape_records WHERE year = \(year)")
        Logger.log("DEBUG: [Landscape] Current Total Records for \(year): \(countRow.first?["c"] ?? 0)")
    ...
    private func detectLandscapeColumns(url: URL) async throws -> (mapping: [String: Int], rowIndex: Int) {
        ...
        guard let data = try handle.read(upToCount: 256000), // Read even more to find headers in very large files
    ...
    private func fastBareMetalStream(url: URL, table: String, mapping: [String: Int], filterEnrollment: Bool, skipRows: Int) throws {
        ...
        data.withUnsafeBytes { (bytes: UnsafeRawBufferPointer) in
            ...
            var fieldOffsets = [(start: Int, len: Int)](repeating: (0, 0), count: 128) // Support wider files
            ...
            Logger.log("DEBUG: [BareMetal] \(table) Ingested \(rowCount) rows")
        }
    ...
    private func detectCPSCColumns(url: URL) async throws -> [String: Int] {
        ...
        for (i, h) in headers.enumerated() {
            ...
            else if c.contains("ssa") && c.contains("county") && c.contains("code") { mapping["ssa_county_code"] = i }
            else if c.contains("fips") && c.contains("county") && c.contains("code") { mapping["fips_county_code"] = i }
